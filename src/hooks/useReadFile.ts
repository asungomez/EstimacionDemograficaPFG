...
import Parser, { FileContents, FileType, ParserDefinition } from "../parser/Parser";
...
const useReadFile = (file: File) => {
  ...
  const readFile = useCallback((format : FileType, config?: ParserDefinition) => {
  ...
  const retry = useCallback((format : FileType, config?: ParserDefinition) => {
    setLoading(true);
    setError(null);
